...
WINLOG="labs/lab13-logging-perf-benchmark/results/sweep-64k-windows.txt"
...
for RPS in 500 300; do
  for COND in N64 E64; do
    RUN="r${RPS}-${COND}"
    echo "=== $RUN 시작 $(date -u +%H:%M:%SZ) ==="
    ensure_rule
    ./scripts/logbench-v4/configure-condition.sh "$COND" >/dev/null 2>&1
    sleep 15
    $SSH "cd ~/logbench && nohup ./.venv/bin/python benchmark.py --url '$URL' --condition '$COND' --run-id '$RUN' --output ~/logbench/results/$RUN --payload-bytes 65536 --rate $RPS --concurrency 20 --warmup-seconds 60 --duration 180 > ~/logbench/$RUN.log 2>&1 & echo started"
