...
> 🔬 **핸즈온 노트북** — [`test-cleanup.ipynb`](./test-cleanup.ipynb) 가 아래 절차를 자동화·가드합니다.
> **Phase 1 인벤토리**(읽기 전용)로 기반 리소스와 랩 아티팩트를 구분해 보여주고,
> **Phase 2 선별 정리**(랩이 만든 제품/구독/백엔드/NV/API/Workbook 만)와 **Phase 3 전체 삭제**는
> 기본 `DRY_RUN = True` 로 **계획만 출력**합니다(실수 방지). 확인 후 플래그를 바꿔 실제 삭제하세요.
> 실제 배포에서 인벤토리 분류(기반 🔒 / 랩 🧪)가 정확함을 라이브 검증했습니다.
