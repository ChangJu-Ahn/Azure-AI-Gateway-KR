...
| 가설 | 판정 | 근거 |
|------|------|------|
| 로깅 비용은 **단건 레이턴시가 아니라 부하 현상** | ✅ **확정** | 단건 서버측 `TotalTime` **0~1ms**로 구성 간 구분 불가(§5) |
| C2: App Insights body 로깅이 **throughput를 떨어뜨린다** | ❌ **미입증(재현 실패)** | 1·2회차 −26%였으나 3회차 뒤집힘; 확정 실험(≥1400RPS)에서 차이 없음(§3) |
| C3(Event Hub)는 C2보다 **감사 능력이 우수**하다 | ✅ **확정(성능 아닌 설계 사실)** | 무손실·200KB·샘플링 무관·불변 아카이브(§4) |

**결론**:
- **성능 축**: 우리가 만든 부하(동시성 제한 ≈1500 RPS, 게이트웨이 CPU **미포화**)에서는 C1/C2/C3 **throughput·응답 차이가 측정되지 않았다.** 문서의 "40~50%↓"는 **CPU 포화**가 전제라 우리 조건에선 미검증(있다/없다 판정 불가).
- **감사 능력 축**: Event Hub(C3)를 쓰는 이유는 **성능이 아니라** 본문 무손실(≤200KB)·샘플링 무관·불변 장기 보존이다. 이는 측정이 아니라 **설계상 확정된 사실**이며, 이 랩(및 Lab 11)이 Event Hub를 쓰는 진짜 근거다.
- **방법론 교훈**: 단발·저부하 측정은 배포별·런별 변동(±수십%)에 취약하다. **반복·워밍업·순서 균형**이 없으면 노이즈를 신호로 오인한다.
