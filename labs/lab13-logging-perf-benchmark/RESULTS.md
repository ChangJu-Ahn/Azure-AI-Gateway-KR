...
### 10.2 CPU 포화 (프로파일 A, body 8KB, knee=4×250)
knee 탐색: 2×250→CPU32%/1322rps · 4×250→CPU61%/2695rps · 6×250→CPU61%/4100rps (CPU가 ~61%에서 정체 = 이 mock 워크로드는 게이트웨이를 60%대까지만 사용).
...
→ **H6 방향성 확인**: **body가 커질수록 C3(EH)의 CPU 비용이 가장 가파르게 증가**(21→26→**37%**). 200KB에서 C3 CPU가 최고(37%)이고 RPS 최저(388) — `log-to-eventhub`가 200KB 전량을 인라인 직렬화·전송하는 비용. 반면 **C2는 8KB 상한이라 body가 커져도 로깅 몫은 평탄**(CPU 18→24→29는 주로 응답 전송 자체 비용).
→ **EH throttle/drop = 0 (전 구간)**: 이 부하(1×100, ~400-600rps)에선 200KB×RPS가 1TU(1MB/s)를 넘겼을 것으로 추정되나 **버퍼링·재시도로 유실 없이 흡수**. 더 센 부하에선 throttle 가능(미도달).
