...
| # | 질문 | 왜 중요한가 |
|---|---|---|
| Q-A | **로깅 목적**이 "일부만 기록해도 되는 모니터링(진단·대시보드)"인가, "모든 로그 무손실 감사(컴플라이언스)"인가? | 저장소 선택의 1차 갈림길 |
...
flowchart TD
    ...
    PURPOSE -->|"무손실 감사<br/>(모든 로그 보존 필수)"| LOSSLESS{무손실 요구}
    ...
    SINK -->|"Azure Monitor 통합<br/>(쿼리·알림·분석)"| AI_100([App Insights<br/>모든 로그 기록 100%])
...
| 무손실 메커니즘 | **모든 로그 기록(100%)로 설정하면 모두 기록** (본 실험 150,178건 무손실) | 정책은 모든 요청에 실행되나 **APIM 큐 한도 도달 시 드롭** |
...
| 컴플라이언스 모든 로그 감사, **중·저부하** | **App Insights 100%** 또는 **Event Hub (안전 범위 내)** | 안전 범위 안이면 둘 다 무손실; 통합 우선 AI, 스트리밍 우선 EH |
| 컴플라이언스 모든 로그 감사, **고부하/큰 본문** | **APIM 등급 상향(v2/Premium) + 데이터 축소 → 그다음 저장소** | 무손실은 저장소가 아니라 게이트웨이 용량이 결정 |
