...
- 환경: Azure API Management(APIM) Developer v1, Korea Central; Basic v2는 비교용으로만 보았다.
- 조건: 로그 저장소는 App Insights와 Event Hub이며, 요청 데이터 크기(payload)는 8KB와 64KB다.
- 기준선: metadata-only 기준선은 App Insights 메타데이터만 기록한 기준선이다.
- 주의: N8/N64 조건은 App Insights diagnostic을 완전히 끈 상태가 아니다; body bytes만 0이다.
- 무손실: 요청 로그가 모두 도착했다는 의미이나 요청 단위 증명은 완료되지 않았다.
- 지표: 드롭은 APIM `EventHubDroppedEvents`, Capacity는 classic v1 부하, Duration은 APIM 서버측 처리시간이다.
- 범위: Developer v1 중심, Developer v1과 Basic v2의 8KB 500 RPS 비교 한 건, 100~500 RPS, 조건별 대부분 한 번만 실행; 1,000 RPS는 테스트하지 않았다.
...
| 400 | 85.5% | 84.5% | 85.5% |
| 500 | 약 89% | 약 89% | 약 87% |
- Capacity 해석: 200 RPS에서 E8 Capacity는 85%였고 N8은 58%, A8은 66%였다; 400~500 RPS에서는 세 조건 모두 상단 범위에 가까워 Capacity만으로 로깅 효과를 분리할 수 없었다; 이 단일 실행들에서 보편 Capacity 임계값을 제시하지 않는다.
...
- **핵심 근거:** EH 스로틀링은 측정창 집계에서 0으로 확인됐고, RPS 구간별 개별 확인은 일부만 기록됐다.
...
- **핵심 근거:** 동일 8KB 500 RPS E8 비교에서 EH 도달 수와 drop 지표가 비대칭으로 수집됐다.
...
1. API 성공 SLO와 로깅-delivery SLO를 분리한다.
2. APIM EH success/drop과 Event Hubs ingress/throttling을 함께 모니터링한다.
3. 운영 목표 peak RPS × logged payload size로 사전 부하 테스트한다.
4. v1은 classic Capacity, v2는 gateway CPU/Memory 계열 지표를 수집한다.
5. 기록할 본문 크기를 최소화하고 필드·요약·해시 중심으로 감사 요구를 재설계한다.
6. 고객 환경에서 재시험해 region, SKU, unit 수, backend 지연, policy 조합을 반영한다.

## 한계

- 조건별 3회 반복 계획은 완료되지 않았고 조건별 대부분 1회, 즉 대부분 한 번만 실행했다.
- 요청 ID 집합, 페이로드 해시, 메시지 크기 대조는 최종 판정 근거로 완료되지 않았다.
- App Insights count 150,178에는 warmup 경계 records가 포함될 수 있다.
- 클라이언트 p95/p99에는 load-generator/TLS artifacts가 섞일 수 있어 서버측 Duration과 구분한다.
- v1/v2 metrics are asymmetric: v2 CPU/메모리, v2 Duration, v2 drop 카운터가 없어 메커니즘은 미검증이다.
- 1,000 RPS는 테스트하지 않았다; production multi-unit/region/backend scenarios도 미검증이다.
