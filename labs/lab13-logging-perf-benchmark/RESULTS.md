...
> **부하**: Python asyncio 고정 도착률(open workload), 8KB/64KB 요청, 100~500 RPS, warmup 후 steady-state 측정.
> **권위 지표**: APIM 서버측 `Capacity`(게이트웨이 부하%), `Duration`(게이트웨이 처리시간), `EventHubDroppedEvents`, EH `IncomingMessages`/`ThrottledRequests`, Log Analytics `AppRequests`. 클라이언트 레이턴시는 참고용.
...
3. **드롭의 근본 원인은 APIM 게이트웨이 처리 능력(SKU)이지 Event Hub 용량(TU)이 아니다.** RPS를 낮추거나 SKU를 올리면 드롭이 사라진다(이중 인과 확정).
...
6. **APIM SKU가 결정적 변수다.** 동일 조건에서 Developer v1은 로그 절반을 잃지만 Basic v2는 무손실이고, 응답 꼬리(p99)도 ~16배 개선된다. **무손실 로깅 = sink 선택이 아니라 게이트웨이 SKU 문제.**
...
**판정: ✅ 지지.** App Insights 로깅은 게이트웨이 처리시간을 무로깅 대비 수 배 증가시킨다(500 RPS에서 약 7~10배). 단 절대값은 여전히 ms 단위로 작다. 공식 문서의 "1,000 RPS 초과 시 throughput 40~50%↓" 경고와 방향 일치(수치 크기는 본 환경 미재현 — 게이트웨이가 다른 병목에 먼저 도달).
...
**판정: ⚠️ 조건부 반박.** "정책이 모든 invocation에 실행된다"는 공식 서술은 맞지만, **APIM이 EH로 실제 전송하는 단계에서 큐 한도로 드롭**하므로 고부하에서 무손실이 보장되지 않는다. 오히려 이 환경에선 **App Insights(100%)가 EH보다 기록 완전성이 높았다.** → "모든 요청 로깅 + 성능"을 동시에 얻으려면 sink 선택이 아니라 **게이트웨이 처리 능력(SKU)**이 관건(질문 3·4로 이어짐).

---

## 질문 3 — EH drop 원인은 APIM에서 찾을 수 있는가? 어떻게?

**가설 H3**: EH 드롭 여부·규모는 APIM 서버측 메트릭으로 직접 계량할 수 있다.

**방법·근거**:
- APIM은 `EventHubDroppedEvents` 메트릭을 emit한다. **공식 정의: "events skipped because of queue size limit reached"** (큐 한도 도달로 스킵).
- 검산: `EventHubTotalEvents`(=Successful, EH로 실제 전송 성공) 와 `EventHubDroppedEvents`(버림)는 **별개 카운터**. EH `IncomingMessages`(도달)와 `Successful`이 일치하면 EH는 무손실.
- 교차검증: E8 500 RPS에서 EH IncomingMessages = APIM Successful, EH ThrottledRequests=0 → **EH는 받은 건 다 저장, 버린 주체는 APIM**.

**판정: ✅ 확정.** EH 드롭은 APIM 서버측 `EventHubDroppedEvents`로 직접 관측 가능하며, EH 메트릭(Incoming/Throttled)과의 대조로 "APIM이 버렸나 vs EH가 못 받았나"를 분리할 수 있다.

---

## 질문 4 — EH Drop의 원인은 무엇으로 봐야 하는가

**가설 H4**: EH 드롭의 근본 원인은 EH 용량(TU)이 아니라 APIM 게이트웨이 처리 능력이다.

**근거 데이터 (이중 인과 증명)**:
- **간접 인과 (부하 축)**: 8KB E8 드롭 = 500(대량) → 400(2,933) → 300(0). RPS↓ → 드롭↓. 이때 EH ThrottledRequests는 항상 0, EH 사용률은 10%(4MB/s ÷ 40TU).
- **직접 인과 (SKU 축)**: 동일 8KB 500 RPS E8에서 **Developer v1 = 약 절반 드롭 vs Basic v2 = 무손실**(EH IncomingMessages 분당 30,000 일정 = 500×60). EH·부하 동일, APIM SKU만 변경.
- 64KB에서 EH 80% 부하(32MB/s)에도 EH Throttled=0 → 대용량에서도 EH는 병목 아님.

**판정: ✅ 확정.** 드롭 원인 = **APIM 게이트웨이 처리 포화**. EH TU 증설은 (본 실험 범위에서) 해결책이 아니다. 게이트웨이 SKU 상향 또는 부하 감소가 해결책.

---

## 질문 5 — payload에 따라 APIM 영향이 있을 수 있는가

**가설 H5**: payload가 커지면 게이트웨이 처리시간이 증가하고, 무손실 가능 RPS가 낮아진다.
...
## 질문 6 — APIM SKU(v1 vs v2)에 따라 로깅 성능·무손실이 달라지는가 (파생 핵심 질문)

**가설 H6**: 게이트웨이 SKU를 올리면 같은 로깅 부하에서 드롭이 줄고 레이턴시가 안정된다.
...
**판정: ✅ 확정 (핵심).** SKU 상향은 (1) EH 드롭을 제거해 **무손실 달성**, (2) 응답 꼬리(p99)를 **약 16배 안정화**. 이것이 질문 4(드롭 원인=게이트웨이)의 **직접 인과 증거**이자, 질문 2(무손실 방법)의 **실질적 해답**이다. → **무손실 감사 로깅의 관건은 sink 선택이 아니라 게이트웨이 SKU 확보.**
...
- 100 RPS: N8=31.5%.
- 결론: "케파 영향이 없다"가 아니라 **"500 RPS에선 이미 포화라 케파 지표로 로깅 영향을 분간할 수 없었다"**. 부하를 낮추면 로깅(특히 EH)이 케파를 확실히 올린다.

### 기타-2: "로깅은 실제 응답속도(처리시간)에 영향을 준다"

**판정: ✅ 확정.** 모든 RPS·payload에서 무로깅 < 로깅 순서 일관. 게이트웨이 Duration이 로깅으로 증가(질문 1·5 데이터). 단 클라이언트 체감 레이턴시는 절대값이 작아(sub-ms~ms) 지배적이지 않음.
...
### 추가-2: v2 아키텍처는 EventHubDroppedEvents 메트릭 체계가 v1과 다를 수 있다
...
**판정: △ 관찰 (미확정).** v2 티어의 메트릭 emit 방식/이름이 classic v1과 다를 가능성. 재현 시 v2 전용 메트릭 정의 확인 필요. **방법론적 함의**: SKU별로 "권위 지표"를 미리 검증해야 한다.
...
**판정: ✅ 지지.** 로깅은 이 병목을 "더 빨리" 드러낼 뿐. 근본 용량은 SKU가 결정. (질문 4와 정합)
...
> **이 환경(Developer v1, 500 RPS)에서는 불가능하다.** log-to-eventhub도 게이트웨이 포화로 로그의 약 절반을 드롭한다. **그러나 게이트웨이 SKU를 Basic v2로 올리면 EH가 무손실로 전건 로깅을 달성한다.** 즉 무손실 감사 로깅의 관건은 sink(App Insights vs EH)가 아니라 **게이트웨이 처리 능력(SKU) 확보**다. EH 용량(TU) 증설은 8KB 기준 해결책이 아니다.
